...
const Quiz1 = ({ resetQuiz }) => {
  ...
  // Wrap handleNext with useCallback to memoize the function and avoid unnecessary re-renders
  ...
  }, [selectedOption, currentQuestion, shuffledQuestions, score]);
  ...
  useEffect(() => {
    if (start && timeLeft > 0) {
      ...
      handleNext(); // Call handleNext when time is up
    }
  }, [timeLeft, start, quizCompleted, handleNext]); // Include handleNext as a dependency
